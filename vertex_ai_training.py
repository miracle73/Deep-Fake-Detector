...
class VertexAIDeepfakeTrainer:
    def __init__(self, project_id: str, bucket_name: str, region: str):
        ...
        self.job_name = f"deepfake-detector-{self.timestamp}"  # Use hyphens for job names
    
    def create_training_config(self):
        """Create optimized training configuration for large dataset"""
        config = {
            'model': {
                'name': 'efficientnet_b4',  # Use underscore format for timm
                'num_classes': 2,
                'pretrained': True,
                'dropout': 0.3
            },
            'training': {
                ...
                'epochs': 50,
                'learning_rate': 0.0001,  # Lower learning rate for large dataset
                'weight_decay': 0.0001,
                'patience': 10,
                'gradient_accumulation_steps': 2,  # For effective batch size of 64
            ...
            'data': {
                ...
                'test_split': 0.1,
                'augmentation': True,
                'cache_data': False  # Don't cache with large dataset
            },
            'optimizer': {
                'name': 'AdamW',
                'betas': [0.9, 0.999],
...
'''
    ...
    def submit_training_job(self, machine_type: str = "n1-highmem-8", 
                           ...
                           accelerator_count: int = 1):  # Reduced to 1 GPU
...
def main():
    ...
    # Initialize trainer
    trainer = VertexAIDeepfakeTrainer(args.project_id, args.bucket_name, args.region)
